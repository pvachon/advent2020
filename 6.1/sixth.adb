...
procedure Sixth is
    ...
    cur_mask : Question_Mask := (others => True);
    next_mask : Question_Mask;
    is_empty : Boolean;
    q_sum : Natural := 0;
    cur_count : Natural := 0;
begin

    Text_IO.Put_Line("Advent of Code: 6.1");
    ...
    while not Text_IO.End_Of_File(file) loop
        ...
        if is_empty then
            ...
            cur_mask := (others => True);
        else
            cur_mask := cur_mask and next_mask;
        end if;
    end loop;
    cur_count := Count_Questions(cur_mask);
    ...
    Text_IO.Put_Line("Total questions where entire group answered yes: " & Natural'Image(q_sum));
